...
核心 CSS、JavaScript、评论背景、加载占位图、等级图标、页脚预加载图和 HLS 脚本已随主题打包，默认不依赖第三方 CDN。旧设置中明确选择远程核心资源（值为 `0`）时，才会读取 `ADDGM/sakura` 的固定发布标签；本地副本始终保留为回退来源。

Google Fonts、阿里图标、主题光标和社交图标属于可选增强，网络不可用时会退化为系统字体或默认样式。Gravatar、QQ 头像、音乐平台、图片上传服务以及用户填写的背景/封面 URL 不由主题镜像。完整入口、来源和校验哈希记录在开发仓库的 Beta.7 资源清单中。
